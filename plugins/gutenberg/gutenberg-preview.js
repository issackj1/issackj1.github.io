...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			urbanism_create_observer( 'urbanism-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				urbanism_gutenberg_editor_init();
			} );
		} else {
		 	urbanism_gutenberg_editor_init();
		}
	...
	function urbanism_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'urbanism_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'urbanism_inited' ) ) {
	...
	// Init on page load
	function urbanism_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = urbanism_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + URBANISM_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + URBANISM_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in URBANISM_STORAGE['override_classes'] ) {
				$obj.addClass( URBANISM_STORAGE['override_classes'][i].replace( '%s', URBANISM_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( urbanism_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && URBANISM_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'urbanism_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			urbanism_remove_observer( 'urbanism-check-editor-styles-wrapper' );
			urbanism_create_observer( 'urbanism-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'urbanism_inited' );
	}
} );
