...
jQuery( document ).ready(
	function() {
		...
		urbanism_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="urbanism_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.urbanism_post_editor' );
						if ( link.length > 0 ) {
							urbanism_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.urbanism_post_editor', function(e) {
						urbanism_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('urbanism_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function urbanism_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
