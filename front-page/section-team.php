<div class="front_page_section front_page_section_team<?php
	$urbanism_scheme = urbanism_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $urbanism_scheme ) && ! urbanism_is_inherit( $urbanism_scheme ) ) {
		echo ' scheme_' . esc_attr( $urbanism_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( urbanism_get_theme_option( 'front_page_team_paddings' ) );
	if ( urbanism_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$urbanism_css      = '';
		$urbanism_bg_image = urbanism_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$urbanism_anchor_icon = urbanism_get_theme_option( 'front_page_team_anchor_icon' );
	$urbanism_anchor_text = urbanism_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( urbanism_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$urbanism_css      = '';
			$urbanism_bg_mask  = urbanism_get_theme_option( 'front_page_team_bg_mask' );
			$urbanism_bg_color_type = urbanism_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $urbanism_bg_color_type ) {
				$urbanism_bg_color = urbanism_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$urbanism_caption = urbanism_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $urbanism_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $urbanism_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $urbanism_caption, 'urbanism_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$urbanism_description = urbanism_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $urbanism_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $urbanism_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $urbanism_description ), 'urbanism_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! urbanism_exists_trx_addons() ) {
						...
						urbanism_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
