...
if ( ! function_exists( 'urbanism_wp_theme_setup' ) ) {

	add_action( 'after_setup_theme', 'urbanism_wp_theme_setup' );
	 ...
	 */
	function urbanism_wp_theme_setup() {

		// Remove macros from title
		add_filter( 'wp_title', 'urbanism_remove_macros_from_site_title' );
		add_filter( 'wp_title_parts', 'urbanism_remove_macros_from_site_title' );
		add_filter( 'document_title_parts', 'urbanism_remove_macros_from_site_title' );

		// Breadcrumbs link 'All posts'
		add_filter( 'post_type_archive_link', 'urbanism_get_template_page_link', 10, 2 );
	}
}


if ( ! function_exists( 'urbanism_is_preview' ) ) {
	/**
	 ...
	 */
	function urbanism_is_preview( $builder = 'any' ) {
		return ( in_array( $builder, array( 'any', 'elm', 'elementor' ) ) && function_exists( 'urbanism_elementor_is_preview' ) && urbanism_elementor_is_preview() )
				||
				( in_array( $builder, array( 'any', 'gb', 'gutenberg' ) ) && function_exists( 'urbanism_gutenberg_is_preview' ) && urbanism_gutenberg_is_preview() );
	}
}


if ( ! function_exists( 'urbanism_get_wp_template_hooks' ) ) {
	/**
	 ...
	 */
	function urbanism_get_wp_template_hooks() {
		return apply_filters( 'urbanism_filter_wp_template_hooks', array(
					'404_template',
					'archive_template',
					'attachment_template',
					'author_template',
...
if ( ! function_exists( 'urbanism_detect_blog_mode' ) ) {
	/**
	 ...
	 * 
	 * For example:
	 *   $pos = urbanism_get_theme_option( 'sidebar_position' );
	 * 
	 ...
	 */
	function urbanism_detect_blog_mode() {
		if ( is_front_page() && ! is_home() ) {
			$mode = 'front';
		} elseif ( is_home() ) {
			$mode = 'home';     // Specify 'blog' if you don't need a separate options for the homepage
		} elseif ( urbanism_is_single() ) {
			$mode = 'post';
		} elseif ( is_page() && ! urbanism_storage_isset( 'blog_archive' ) ) {
			$mode = 'page';
		} elseif ( is_category() ) {
			$mode = 'category';
		} elseif ( is_tag() ) {
		...
		return apply_filters( 'urbanism_filter_detect_blog_mode', $mode );
	}
}

if ( ! function_exists( 'urbanism_is_blog_mode_custom' ) ) {
	/**
	 ...
	 */
	function urbanism_is_blog_mode_custom() {
		return ! in_array( urbanism_storage_get( 'blog_mode' ), apply_filters( 'urbanism_filter_blog_mode_standard', array(
...
if ( ! function_exists( 'urbanism_get_current_mode_image' ) ) {
	/**
	 ...
	 */
	function urbanism_get_current_mode_image( $default = '' ) {
		if ( is_category() || is_tax() ) {
			$img = urbanism_get_term_image();
			if ( '' != $img ) {
				$default = $img;
			}
		} elseif ( urbanism_is_singular() || urbanism_storage_isset( 'blog_archive' ) ) {
		...
		return urbanism_clear_thumb_size( $default );
	}
}

if ( ! function_exists( 'urbanism_blog_archive_start' ) ) {
	/**
	 ...
	 */
	function urbanism_blog_archive_start() {
		$main_post = urbanism_storage_get( 'blog_archive_template_post' );
		if ( is_object( $main_post ) ) {
			...
			$urbanism_content            = '';
			$urbanism_blog_archive_mask  = '%%CONTENT%%';
			$urbanism_blog_archive_subst = sprintf( '<div class="blog_archive">%s</div>', $urbanism_blog_archive_mask );
			$urbanism_content            = apply_filters( 'the_content', get_the_content() );
			...
			if ( '' != $urbanism_content ) {
				$urbanism_pos = stripos( $urbanism_content, $urbanism_blog_archive_mask );
				if ( false !== $urbanism_pos ) {
					$urbanism_content = preg_replace( '/(\<p\>\s*)?' . $urbanism_blog_archive_mask . '(\s*\<\/p\>)/i', $urbanism_blog_archive_subst, $urbanism_content );
				} else {
					$urbanism_content .= $urbanism_blog_archive_subst;
				}
				$urbanism_content = explode( $urbanism_blog_archive_mask, $urbanism_content );
				// Display first part
				urbanism_show_layout( apply_filters( 'urbanism_filter_blog_archive_start', $urbanism_content[0] ) );
				// And store second part
				urbanism_storage_set( 'blog_archive_end', $urbanism_content[1] );
			}
...
if ( ! function_exists( 'urbanism_blog_archive_end' ) ) {
	/**
	 * Display the end blog archive template.
	 * 
	 * Output a second part (stored in the previous call of urbanism_blog_archive_start).
	 */
	function urbanism_blog_archive_end() {
		$html = urbanism_storage_get( 'blog_archive_end' );
		if ( '' != $html ) {
			// Display second part of template content
			urbanism_show_layout( apply_filters( 'urbanism_filter_blog_archive_end', $html ) );
		}
	}
}

if ( ! function_exists( 'urbanism_blog_archive_get_template' ) ) {
	/**
	 ...
	 */
	function urbanism_blog_archive_get_template( $blog_style = '' ) {
		if ( empty( $blog_style ) ) {
			$blog_style = urbanism_get_theme_option( 'blog_style' );
		}
		$parts   = explode( '_', $blog_style );
		$archive = 'index';
		if ( urbanism_storage_isset( 'blog_styles', $parts[0], 'archive' ) ) {
			$archive = urbanism_storage_get_array( 'blog_styles', $parts[0], 'archive' );
		}
		return apply_filters( 'urbanism_filter_blog_archive_template', $archive, $blog_style );
	}
}

if ( ! function_exists( 'urbanism_blog_item_get_template' ) ) {
	/**
	 ...
	 *                           or an empty string if not detected.
	 */
	function urbanism_blog_item_get_template( $blog_style = '' ) {
		if ( empty( $blog_style ) ) {
			$blog_style = urbanism_get_theme_option( 'blog_style' );
		}
		$parts = explode( '_', $blog_style );
		...
		} elseif ( urbanism_storage_isset( 'blog_styles', $parts[0], 'item' ) ) {
			$item = urbanism_storage_get_array( 'blog_styles', $parts[0], 'item' );
		} else {
...
if ( ! function_exists( 'urbanism_get_post_id' ) ) {
	/**
	 ...
	 */
	function urbanism_get_post_id( $args = array() ) {
		$args  = array_merge(
			array(
				'posts_per_page' => 1,
			), $args
...
if ( ! function_exists( 'urbanism_get_post_content' ) ) {
	/**
	 ...
	 */
	function urbanism_get_post_content( $apply_filters = false ) {
		global $post;
...
if ( ! function_exists( 'urbanism_filter_post_content' ) ) {
	/**
	 ...
	 */
	function urbanism_filter_post_content( $content ) {
		$content = apply_filters( 'urbanism_filter_post_content', $content );
		global $wp_embed;
...
if ( ! function_exists( 'urbanism_get_template_page_id' ) ) {
	/**
	 ...
	 */
	function urbanism_get_template_page_id( $args = array() ) {
		$args   = array_merge(
			array(
				'template'   => 'blog.php',
				'post_type'  => 'post',
		...
		if ( ! empty( $args['post_type'] ) ) {
			$q_args['meta_query'][] = array(
				'key'     => 'urbanism_options_post_type',
		...
		if ( '' !== $args['parent_cat'] ) {
			$q_args['meta_query'][] = array(
				'key'     => 'urbanism_options_parent_cat',
				'value'   => $args['parent_cat'] > 0 ? $args['parent_cat'] : 1,
				'compare' => $args['parent_cat'] > 0 ? '=' : '<',
			);
		}
		return urbanism_get_post_id( $q_args );
	}
}

if ( ! function_exists( 'urbanism_get_template_page_link' ) ) {
	/**
	 ...
	 * 
	 * Handler of the add_filter('post_type_archive_link', 'urbanism_get_template_page_link', 10, 2 );
	 * 
	 ...
	 */
	function urbanism_get_template_page_link( $link = '', $post_type = '' ) {
		if ( ! empty( $post_type ) ) {
			$id = urbanism_get_template_page_id(
				array(
					'post_type'  => $post_type,
					'parent_cat' => 0,
				)
...
if ( ! function_exists( 'urbanism_get_posts_archive_template' ) ) {

	add_filter( 'archive_template', 'urbanism_get_posts_archive_template', 100 );
	 ...
	 */
	function urbanism_get_posts_archive_template( $template ) {
		if ( urbanism_get_theme_option( 'use_blog_archive_pages' ) ) {
			if ( is_post_type_archive() ) {
				$obj = get_queried_object();
				if ( ! empty( $obj->name ) ) {
					$templates = get_option( 'urbanism_blog_archive_templates' );
					if ( ! empty( $templates[ $obj->name ] ) ) {
						$template = urbanism_redirect_to_archive_template( $template, $templates[ $obj->name ] );
					}
				}
			} else {
				$template = urbanism_get_tax_archive_template( $template );
			}
		}
		return $template;
	}	
...
if ( ! function_exists( 'urbanism_get_tax_archive_template' ) ) {

	add_filter( 'category_template', 'urbanism_get_tax_archive_template', 100 );
	add_filter( 'taxonomy_template', 'urbanism_get_tax_archive_template', 100 );
	 ...
	 * @return string          A filtered path to the template
	 */
	function urbanism_get_tax_archive_template( $template ) {
		if ( urbanism_get_theme_option( 'use_blog_archive_pages' ) && ( is_category() || is_tag() || is_tax() ) ) {
			$obj = get_queried_object();
			global $wp_query;
			...
			if ( $pt && $tax && $term ) {
				$pt_tax  = urbanism_get_post_type_taxonomy( $pt );
				if ( $pt_tax == $tax ) {
					$templates = get_option( 'urbanism_blog_archive_templates' );
					if ( ! empty( $templates[ "{$pt}_{$tax}_{$term}" ] ) ) {
						$template = urbanism_redirect_to_archive_template( $template, $templates[ "{$pt}_{$tax}_{$term}" ] );
					} else {
						$found = false;
						do {
							...
							if ( ! empty( $templates[ "{$pt}_{$tax}_{$parent}" ] ) ) {
								$template = urbanism_redirect_to_archive_template( $template, $templates[ "{$pt}_{$tax}_{$parent}" ], $term );
								$found = true;
								break;
							} else {
						...
						if ( ! $found && ! empty( $templates[ "{$pt}" ] ) ) {
							$template = urbanism_redirect_to_archive_template( $template, $templates[ "{$pt}" ], $term );
						}
					}
				}
			}
...
if ( ! function_exists( 'urbanism_redirect_to_archive_template' ) ) {
	/**
	 ...
	 */
	function urbanism_redirect_to_archive_template( $template, $page_id, $term = false ) {
		// Store page number
		...
		// Load page options
		urbanism_override_theme_options( null, $page_id );
		// Override parent category
		if ( $term > 0 ) {
			urbanism_storage_set_array( 'options_meta', 'parent_cat', $term );
		}
		return urbanism_get_file_dir( 'blog.php' );
	}
}

if ( ! function_exists( 'urbanism_save_archive_template' ) ) {

	add_action( 'save_post', 'urbanism_save_archive_template', 11 );
	 ...
	 */
	function urbanism_save_archive_template( $post_id ) {
		// verify nonce
		if ( ! wp_verify_nonce( urbanism_get_value_gp( 'override_options_nonce' ), admin_url() ) ) {
			return $post_id;
		}
		...
		if ( 'page' == $post_type ) {
			$meta = get_post_meta( $post_id, 'urbanism_options', true );
			...
			if ( 'blog.php' == $page_template ) {
				...
				$tax  = urbanism_get_post_type_taxonomy( $pt );
				$term = isset( $meta['parent_cat'] ) ? $meta['parent_cat'] : 0;
				$templates = urbanism_array_delete_by_value( $templates, $post_id );
				$templates[ $pt . ( $term > 0 ? "_{$tax}_{$term}" : '' ) ] = $post_id;
			} else {
				$templates = urbanism_array_delete_by_value( $templates, $post_id );
			}
			update_option( 'urbanism_blog_archive_templates', $templates );
		}
	}
}

// Delete page from templates
if ( ! function_exists( 'urbanism_delete_archive_template' ) ) {

	add_action( 'delete_post', 'urbanism_delete_archive_template', 11 );
	 ...
	 */
	function urbanism_delete_archive_template( $post_id ) {
		$templates = get_option( 'urbanism_blog_archive_templates' );
		if ( is_array( $templates ) ) {
			$templates = urbanism_array_delete_by_value( $templates, $post_id );
			update_option( 'urbanism_blog_archive_templates', $templates );
...
if ( ! function_exists( 'urbanism_get_hash_link' ) ) {
	 ...
	 */
	function urbanism_get_hash_link( $hash ) {
		if ( 0 !== strpos( $hash, 'http' ) ) {
			if ( '#' != $hash[0] ) {
				$hash = '#' . $hash;
			}
			if ( is_customize_preview() ) {
				$url = urbanism_get_current_url();
				$pos = strpos( $url, '#' );
				if ( false !== $pos ) {
...
if ( ! function_exists( 'urbanism_get_current_url' ) ) {
	/**
	 * Return an URL to the current page.
	 * 
	 * @return string  A current page URL.
	 */
	function urbanism_get_current_url() {
		return add_query_arg( array() );
	}
}

if ( ! function_exists( 'urbanism_check_url' ) ) {
	/**
	 ...
	 */
	function urbanism_check_url( $val ) {
		if ( ! is_array( $val ) ) {
			$val = array( $val );
		}
		$rez = false;
...
if ( ! function_exists( 'urbanism_attachment_url_to_postid' ) ) {
	/**
	 ...
	 */
	function urbanism_attachment_url_to_postid( $url ) {
		static $images = array();
		if ( ! isset( $images[ $url ] ) ) {
			$images[ $url ] = attachment_url_to_postid( urbanism_clear_thumb_size( $url, false ) );
		}
		return $images[ $url ];
	}
}

if ( ! function_exists( 'urbanism_remove_macros_from_site_title' ) ) {
	/**
	 ...
	 */
	function urbanism_remove_macros_from_site_title( $title ) {
		if ( is_array( $title ) ) {
			foreach ( $title as $k => $v ) {
				$title[ $k ] = urbanism_remove_macros( $v );
			}
		} else {
			$title = urbanism_remove_macros( $title );
		}
		return $title;
	}
}

if ( ! function_exists( 'urbanism_get_blog_title' ) ) {
	/**
	 * Return a blog title for the current blog mode.
	 *
	 * @return string  A blog title.
	 */
	function urbanism_get_blog_title() {

		if ( is_front_page() ) {
			$title = esc_html__( 'Home', 'urbanism' );
		} elseif ( is_home() ) {
			$title = esc_html__( 'All Posts', 'urbanism' );
		} elseif ( is_author() ) {
			$curauth = ( get_query_var( 'author_name' ) ) ? get_user_by( 'slug', get_query_var( 'author_name' ) ) : get_userdata( get_query_var( 'author' ) );
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Author page: %s', 'urbanism' ), $curauth->display_name );
		} elseif ( is_404() ) {
			$title = esc_html__( 'URL not found', 'urbanism' );
		} elseif ( is_search() ) {
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Search: %s', 'urbanism' ), get_search_query() );
		} elseif ( is_day() ) {
			// Translators: Add the queried date to the title
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'urbanism' ), get_the_date() );
		} elseif ( is_month() ) {
			// Translators: Add the queried month to the title
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'urbanism' ), get_the_date( 'F Y' ) );
		} elseif ( is_year() ) {
			// Translators: Add the queried year to the title
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'urbanism' ), get_the_date( 'Y' ) );
		} elseif ( is_category() ) {
			$title = single_cat_title( '', false );
		} elseif ( is_tag() ) {
			// Translators: Add the tag's name to the title
			$title = sprintf( esc_html__( 'Tag: %s', 'urbanism' ), single_tag_title( '', false ) );
		} elseif ( is_tax() ) {
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'urbanism' ), get_the_title() );
		} elseif ( urbanism_is_single() || is_page() ) {
			$title = get_the_title();
		} else {
			$title = get_the_title();
		}
		return apply_filters( 'urbanism_filter_get_blog_title', $title );
	}
}

if ( ! function_exists( 'urbanism_get_post_categories' ) ) {
	/**
	 ...
	 */
	function urbanism_get_post_categories( $delimiter = ', ', $id = false, $links = true ) {
		return urbanism_get_post_terms( $delimiter, $id, 'category', $links );
	}
}

if ( ! function_exists( 'urbanism_get_post_terms' ) ) {
	/**
	 ...
	 */
	function urbanism_get_post_terms( $delimiter = ', ', $id = false, $taxonomy = 'category', $links = true ) {
		$output = '';
		if ( empty( $id ) ) {
			$id = get_the_ID();
		}
		if ( empty( $taxonomy ) ) {
			$taxonomy = urbanism_get_post_type_taxonomy( get_post_type( $id ) );
		}
				...
				$output .= ( ! empty( $output )
							...
							. ( $links
								? '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '"'
										...
										. ' title="' . sprintf( esc_attr__( 'View all posts in %s', 'urbanism' ), $term->name ) . '"'
										. '>'
								: '<span>'
								)
								. apply_filters( 'urbanism_filter_term_name', $term->name, $term )
							. ( $links
								? '</a>'
								: '</span>'
								);
...
if ( ! function_exists( 'urbanism_get_post_type_taxonomy' ) ) {
	/**
	 ...
	 */
	function urbanism_get_post_type_taxonomy( $post_type = '' ) {
		if ( empty( $post_type ) ) {
		...
		return apply_filters( 'urbanism_filter_post_type_taxonomy', $tax, $post_type );
	}
}

if ( ! function_exists( 'urbanism_get_edited_post_id' ) ) {
	/**
	 ...
	 */
	function urbanism_get_edited_post_id() {
		$id = false;
		if ( is_admin() ) {
			$url = urbanism_get_current_url();
			if ( strpos( $url, 'post.php' ) !== false ) {
				if ( urbanism_get_value_gp( 'action' ) == 'edit' ) {
					$post_id = urbanism_get_value_gp( 'post' );
					if ( 0 < $post_id ) {
						$id = $post_id;
					}
				}
...
if ( ! function_exists( 'urbanism_get_edited_post_type' ) ) {
	/**
	 ...
	 */
	function urbanism_get_edited_post_type() {
		$pt = '';
		if ( is_admin() ) {
			$url = urbanism_get_current_url();
			if ( strpos( $url, 'post.php' ) !== false ) {
				if ( in_array( urbanism_get_value_gp( 'action' ), array( 'edit', 'elementor' ) ) ) {
					$id = urbanism_get_value_gp( 'post' );
					if ( 0 < $id ) {
				...
				$pt = urbanism_get_value_gp( 'post_type' );
			}
		}
		return $pt;
	}
...
if ( ! function_exists( 'urbanism_is_post_edit' ) ) {
	/**
	 * Detect if a current mode is "Edit post" or "Site editor".
	 *
	 * @return bool Return true if current mode is "Edit post" or "Edit site".
	 */
	function urbanism_is_post_edit() {
		return ( urbanism_check_url( 'post.php' ) && ! empty( $_GET['action'] ) && $_GET['action'] == 'edit')
				||
				urbanism_check_url( 'post-new.php' )
				||
				( urbanism_check_url( '/block-renderer/trx-addons/' ) && ! empty( $_GET['context'] ) && $_GET['context'] == 'edit' )
				||
				( urbanism_check_url( 'admin.php' ) && ! empty( $_GET['page'] ) && $_GET['page'] == 'gutenberg-edit-site' )
				||
				( urbanism_check_url( 'site-editor.php' ) && ! empty( $_GET['postType'] ) )	// || $_GET['postType'] == 'wp_template' ) )
				||
				urbanism_check_url( 'widgets.php' );
	}
}

if ( ! function_exists( 'urbanism_is_singular' ) ) {
	/**
	 ...
	 */
	function urbanism_is_singular( $type = '' ) {
		global $wp_query;
		return apply_filters( 'urbanism_filter_is_singular', ! empty( $wp_query->queried_object->ID ) && is_singular( $type ), $type );
	}
}

if ( ! function_exists( 'urbanism_is_single' ) ) {
	/**
	 * Detect if the current post is a single post.
	 *
	 * @return bool  true if the current mode is a single post.
	 */
	function urbanism_is_single() {
		global $wp_query;
		return apply_filters( 'urbanism_filter_is_single', ! empty( $wp_query->queried_object->ID ) && is_single() );
	}
}

if ( ! function_exists( 'urbanism_add_seo_itemprops' ) ) {
	/**
	 * Add SEO params to the article tag.
	 */
	function urbanism_add_seo_itemprops() {
		if ( urbanism_is_on( urbanism_get_theme_option( 'seo_snippets' ) ) ) {
			...
			?>" 
			itemtype="<?php echo esc_attr( urbanism_get_protocol( true ) ); ?>//schema.org/<?php echo esc_attr( urbanism_get_markup_schema() ); ?>" 
...
if ( ! function_exists( 'urbanism_add_seo_snippets' ) ) {
	/**
	 * Add SEO meta to the post from the template 'templates/seo.php'
	 */
	function urbanism_add_seo_snippets() {
		if ( urbanism_is_on( urbanism_get_theme_option( 'seo_snippets' ) ) ) {
...
if ( ! function_exists( 'urbanism_get_nav_menu' ) ) {
	/**
	 ...
	 */
	function urbanism_get_nav_menu( $location = '', $menu = '', $depth = 0, $custom_walker = false ) {
		static $list = array();
		$slug = $location . '_' . $menu;
		if ( empty( $list[ $slug ] ) ) {
			$list[ $slug ] = esc_html__( 'You are trying to use a menu inserted in himself!', 'urbanism' );
			$args          = array(
								'menu'            => empty( $menu ) || 'default' == $menu || urbanism_is_inherit( $menu ) ? '' : $menu,
								'container'       => '',
								'container_class' => '',
								'container_id'    => '',
								...
								'before'          => '',
								'after'           => '',
								'link_before'     => urbanism_get_theme_setting( 'wrap_menu_items_with_span' ) ? '<span>' : '',
								'link_after'      => urbanism_get_theme_setting( 'wrap_menu_items_with_span' ) ? '</span>' : '',
								'depth'           => $depth,
							);
			if ( ! empty( $location ) ) {
				$args['theme_location'] = $location;
			}
			if ( $custom_walker && class_exists( 'urbanism_custom_menu_walker' ) ) {
				$args['walker'] = new urbanism_custom_menu_walker;
			}
			$list[ $slug ] = wp_nav_menu( apply_filters( 'urbanism_filter_get_nav_menu_args', $args ) );
		}
		return apply_filters( 'urbanism_filter_get_nav_menu', $list[ $slug ], $location, $menu );
	}
}

if ( ! function_exists( 'urbanism_remove_empty_spaces_between_menu_items' ) ) {

    add_action( 'wp_nav_menu', 'urbanism_remove_empty_spaces_between_menu_items', 98, 2 );
	 ...
	 */
	function urbanism_remove_empty_spaces_between_menu_items( $html = '', $args = array() ) {
		return preg_replace(
							array( "/>[\r\n\s]*<li/", "/>[\r\n\s]*<\\/ul>/" ),
							array( "><li", "></ul>" ),
							$html
...
if ( ! function_exists( 'urbanism_remove_empty_menu_items' ) ) {

	add_action( 'wp_nav_menu', 'urbanism_remove_empty_menu_items', 99, 2 );
	 ...
	 * @return string       A clean string.
	 */
	function urbanism_remove_empty_menu_items( $html = '', $args = array() ) {
		return urbanism_get_theme_setting( 'remove_empty_menu_items' )
					? preg_replace(
							"/<li[^>]*>[\r\n\s]*<a[^>]*>[\r\n\s]*(<span>[\r\n\s]*<\\/span>[\r\n\s]*)?<\\/a>[\r\n\s]*<\\/li>/",
							"",
							$html
...
//
if ( ! function_exists( 'urbanism_new_main_query' ) ) {
	/**
	 ...
	 */
	function urbanism_new_main_query( $args ) {
		...
		if ( ! empty( $args['post_type'] ) || ! empty( $args['category'] ) ) {
			$query_args  = urbanism_query_add_posts_and_cats( $query_args, $args['post_ids'], $args['post_type'], $args['category'] );
		}
		if ( $args[ 'page' ] > 1 ) {
...
if ( ! function_exists( 'urbanism_query_add_sort_order' ) ) {
	/**
	 ...
	 */
	function urbanism_query_add_sort_order( $args, $orderby = 'date', $order = 'desc' ) {
		if ( ! empty( $orderby ) && ( empty( $args['orderby'] ) || 'none' != $orderby ) ) {
			$q          = apply_filters( 'urbanism_filter_add_sort_order', array(), $orderby, $order );
		...
		return apply_filters( 'urbanism_filter_add_sort_order_args', $args, $orderby, $order );
	}
}

if ( ! function_exists( 'urbanism_query_add_posts_and_cats' ) ) {
	/**
	 ...
	 */
	function urbanism_query_add_posts_and_cats( $args, $ids = '', $post_type = '', $cat = '', $taxonomy = '' ) {
		if ( ! empty( $ids ) ) {
			...
			if ( ! empty( $cat ) ) {
				...
				if ( empty( $taxonomy ) ) {
					$taxonomy = urbanism_get_post_type_taxonomy( $post_type );
				}
...
if ( ! function_exists( 'urbanism_query_add_filters' ) ) {
	/**
	 ...
	 */
	function urbanism_query_add_filters( $args, $filters = false ) {
		if ( ! empty( $filters ) ) {
			if ( ! is_array( $filters ) ) {
...
if ( ! function_exists( 'urbanism_create_widgets_area' ) ) {
	/**
	 ...
	 */
	function urbanism_create_widgets_area( $name, $add_classes = '' ) {
		$widgets_name = urbanism_get_theme_option( $name );
		if ( ! urbanism_is_off( $widgets_name ) && is_active_sidebar( $widgets_name ) ) {
			urbanism_storage_set( 'current_sidebar', $name );
			ob_start();
			...
			if ( ! empty( $out ) ) {
				...
				$need_columns = strpos( $out, 'columns_wrap' ) === false && apply_filters( 'urbanism_filter_widgets_area_need_columns', 'footer' == $name, $name, $out );
				if ( $need_columns ) {
					$columns = apply_filters( 'urbanism_filter_widgets_area_columns', min( 4, max( 1, urbanism_tags_count( $out, 'aside' ) ) ), $name );
				...
				<div class="<?php echo esc_attr( $name ); ?> <?php echo esc_attr( $name ); ?>_wrap widget_area">
					<?php do_action( 'urbanism_action_before_sidebar_wrap' ); ?>
					<div class="<?php echo esc_attr( $name ); ?>_inner <?php echo esc_attr( $name ); ?>_inner widget_area_inner">
						<?php
						do_action( 'urbanism_action_before_sidebar' );
						urbanism_show_layout(
							$out,
							true == $need_columns ? '<div class="columns_wrap">' : '',
							true == $need_columns ? '</div>' : ''
						);
						do_action( 'urbanism_action_after_sidebar' );
						?>
					</div>
					<?php do_action( 'urbanism_action_after_sidebar_wrap' ); ?>
				</div>
				<?php
			}
		}
...
if ( ! function_exists( 'urbanism_sidebar_present' ) ) {
	/**
	 ...
	 */
	function urbanism_sidebar_present() {
		global $wp_query;
		$sidebar_position = urbanism_get_theme_option( 'sidebar_position' );
		$sidebar_type     = urbanism_get_theme_option( 'sidebar_type' );
		$sidebar_name     = urbanism_get_theme_option( 'sidebar_widgets' );
		return apply_filters(
			'urbanism_filter_sidebar_present',
			! urbanism_is_off( $sidebar_position )
				&& (
						( 'default' == $sidebar_type && ! urbanism_is_off( $sidebar_name ) && is_active_sidebar( $sidebar_name ) )
						||
						( 'custom' == $sidebar_type && urbanism_is_layouts_available() )
					)
				&& ! is_404()
...
if ( ! function_exists( 'urbanism_get_content_width' ) ) {
	/**
	 ...
	 */
	function urbanism_get_content_width() {
		$pg_width = apply_filters( 'urbanism_filter_content_width', urbanism_get_theme_option( 'page_width' ) );
		$sb_width = 0;
		$sb_gap   = 0;
		if ( urbanism_sidebar_present() ) {
			...
			if ( 'default' == $urbanism_sidebar_type ) {
				...
				if ( is_active_sidebar( $urbanism_sidebar_name ) ) {
					$sb_width = urbanism_get_theme_option( 'sidebar_width' );
					$sb_gap   = urbanism_get_theme_option( 'sidebar_gap' );
				}
			} else {
				$sb_width = urbanism_get_theme_option( 'sidebar_width' );
				$sb_gap   = urbanism_get_theme_option( 'sidebar_gap' );
...
if ( ! function_exists( 'urbanism_add_inline_css_class' ) ) {
	/**
	 ...
	 */
	function urbanism_add_inline_css_class( $css, $suffix = '' ) {
		$class_name = urbanism_generate_id( 'urbanism_inline_' );
		urbanism_add_inline_css(
			sprintf(
				'.%s%s{%s}',
				$class_name,
				! empty( $suffix )
...
if ( ! function_exists( 'urbanism_add_inline_css' ) ) {
	/**
...
if ( ! function_exists( 'urbanism_get_date' ) ) {
	/**
	 ...
	 */
	function urbanism_get_date( $dt = '', $format = '' ) {
		if ( '' == $dt ) {
			$dt = get_the_time( 'U' );
		}
		if ( date( 'U' ) - $dt > intval( urbanism_get_theme_option( 'time_diff_before' ) ) * 24 * 3600 ) {
			$dt = date_i18n( '' == $format ? get_option( 'date_format' ) : $format, $dt );
		} else {
			// Translators: Add the human-friendly date difference
			$dt = sprintf( esc_html__( '%s ago', 'urbanism' ), human_time_diff( $dt, current_time( 'timestamp' ) ) );
		}
		return $dt;
	}
}
...
if ( ! function_exists( 'urbanism_lazy_load_off' ) ) {
	/**
	 * Disable a lazy load behaviour for rest images on the current page
	 */
	function urbanism_lazy_load_off() {
		if ( ! urbanism_lazy_load_is_off() ) {
			add_filter( 'wp_lazy_loading_enabled', 'urbanism_lazy_load_disabled' );
		}
	}
}

if ( ! function_exists( 'urbanism_lazy_load_disabled' ) ) {
	/**
	 ...
	 */
	function urbanism_lazy_load_disabled() {
		return false;
	}
}

if ( ! function_exists( 'urbanism_lazy_load_on' ) ) {
	/**
	 * Enable a lazy load behaviour for rest images on the current page
	 */
	function urbanism_lazy_load_on() {
		if ( urbanism_lazy_load_is_off() ) {
			remove_action( 'wp_lazy_loading_enabled', 'urbanism_lazy_load_disabled' );	// Is equal to rf
		}
	}
}

if ( ! function_exists( 'urbanism_lazy_load_is_off' ) ) {
	/**
	 * Return a current state of the lazy load behaviour
	 *
	 * @return bool  A current lazy load state
	 */
	function urbanism_lazy_load_is_off() {
		return has_filter( 'wp_lazy_loading_enabled', 'urbanism_lazy_load_disabled' );
	}
}
...
if ( ! function_exists( 'urbanism_get_markup_schema' ) ) {
	/**
	 * Return a structured data markup schema for the current page.
	 *
	 * @return string  A schema of the current page.
	 */
	function urbanism_get_markup_schema() {
		if ( urbanism_is_single() ) {                                        // Is single post
			$type = 'Article';
...
if ( ! function_exists( 'urbanism_get_privacy_text' ) ) {
	/**
	 * Return a text for the Privacy Policy checkbox.
	 *
	 * @return string  A label text.
	 */
	function urbanism_get_privacy_text() {
		$page         = get_option( 'wp_page_for_privacy_policy' );
		$privacy_text = urbanism_get_theme_option( 'privacy_text' );
		return apply_filters(
			'urbanism_filter_privacy_text',
			wp_kses(
				$privacy_text
				. ( ! empty( $page ) && ! empty( $privacy_text )
					// Translators: Add url to the Privacy Policy page
					? ' ' . sprintf( __( 'For further details on handling user data, see our %s.', 'urbanism' ),
						'<a href="' . esc_url( get_permalink( $page ) ) . '" target="_blank">'
						. __( 'Privacy Policy', 'urbanism' )
						. '</a>' )
					: ''
					),
				'urbanism_kses_content'
				)
			);
	}
}
...
if ( ! function_exists( 'urbanism_kses_allowed_html' ) ) {
	add_filter( 'wp_kses_allowed_html', 'urbanism_kses_allowed_html', 10, 2 );
	/**
	 * Return a theme-specific tags list with allowed attributes for using in the wp_kses() calls
	 * ( if $context is equal to 'urbanism_kses_content' or 'trx_addons_kses_content' ).
	 *
	 * Hooks:
	 *
	 * add_filter( 'wp_kses_allowed_html', 'urbanism_kses_allowed_html', 10, 2 );
	 ...
	 */
	function urbanism_kses_allowed_html( $tags, $context ) {
		if ( in_array( $context, array( 'urbanism_kses_content', 'trx_addons_kses_content' ) ) ) {
			$tags = array( 
...
if ( ! function_exists( 'urbanism_verify_nonce' ) ) {
	/**
	 ...
	 */
	function urbanism_verify_nonce( $nonce = 'nonce', $mask = '' ) {
		if ( empty( $mask ) ) {
			$mask = admin_url('admin-ajax.php');
		}
		if ( ! wp_verify_nonce( urbanism_get_value_gp( $nonce ), $mask ) ) {
			urbanism_forbidden();
		}
	}
}

if ( ! function_exists( 'urbanism_exit' ) ) {
	/**
	 ...
	 */
	function urbanism_exit( $message = '', $title = '', $code = 200 ) {
		wp_die( $message, $title, array( 'response' => $code, 'exit' => empty( $message ) && empty( $title ) ) );
	}
}

if ( ! function_exists( 'urbanism_forbidden' ) ) {
	/**
	 ...
	 * @param string $title    Optional. Title to output while exit.
	 */
	function urbanism_forbidden( $message = '', $title = '' ) {
		urbanism_exit( $message, $title, 403 );
	}
}

if ( ! function_exists( 'urbanism_ajax_response' ) ) {
	/**
	 * Send an ajax response and exit.
	 *
	 * @param mixed $response  A response to send to the client.
	 */
	function urbanism_ajax_response( $response ) {
...
if ( ! function_exists( 'urbanism_get_upgrade_url' ) ) {
	/**
	 ...
	 */
	function urbanism_get_upgrade_url( $params = array() ) {
		$theme_folder = get_template();
		$theme_slug   = apply_filters( 'urbanism_filter_original_theme_slug', $theme_folder );
		$default      = array(
							'action'     => '',
		...
		if ( ! empty( $params['action'] ) && ! in_array( $params['action'], array( 'info_skins' ) ) ) {
			$default['key']        = '';
			$default['src']        = urbanism_get_theme_pro_key();
			$default['theme_name'] = wp_get_theme( $theme_folder )->get( 'Name' );
			$default['domain']     = urbanism_remove_protocol_from_url( get_home_url(), true );
		...
		return urbanism_add_to_url( trailingslashit( urbanism_storage_get( 'theme_upgrade_url' ) ) . 'upgrade.php', $params );
	}
}

if ( ! function_exists( 'urbanism_get_upgrade_data' ) ) {
	/**
	 ...
	 */
	function urbanism_get_upgrade_data( $params = array() ) {
		$url = urbanism_get_upgrade_url( $params );
		$result = function_exists( 'trx_addons_fgc' ) ? trx_addons_fgc( $url ) : urbanism_fgc( $url );
		if ( is_serialized( $result ) ) {
			try {
				$result = urbanism_unserialize( $result );
			} catch ( Exception $e ) {
			}
		}
		if ( ! isset( $result['error'] ) || ! isset( $result['data'] ) ) {
			$result = array(
				'error' => esc_html__( 'Unrecognized server answer!', 'urbanism' ),
				'data'  => ''
			);
		}
		return $result;
...
if ( ! function_exists( 'urbanism_allow_upload_archives' ) ) {
	/**
	 * Allow WordPress to upload archives before get an upgrade package from the upgrade server.
	 */
	function urbanism_allow_upload_archives() {
...
if ( ! function_exists( 'urbanism_disallow_upload_archives' ) ) {
	/**
	 * Disallow WordPress to upload archives after the upgrade package received.
	 */
	function urbanism_disallow_upload_archives() {
