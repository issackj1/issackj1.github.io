...
if ( ! function_exists( 'urbanism_storage_get' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_get( $var_name, $default = '' ) {
		global $URBANISM_STORAGE;
		return isset( $URBANISM_STORAGE[ $var_name ] ) ? $URBANISM_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'urbanism_storage_set' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_set( $var_name, $value ) {
		global $URBANISM_STORAGE;
		$URBANISM_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'urbanism_storage_empty' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $URBANISM_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $URBANISM_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $URBANISM_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $URBANISM_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'urbanism_storage_isset' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $URBANISM_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $URBANISM_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $URBANISM_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $URBANISM_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'urbanism_storage_unset' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $URBANISM_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $URBANISM_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $URBANISM_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $URBANISM_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'urbanism_storage_inc' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_inc( $var_name, $value = 1 ) {
		global $URBANISM_STORAGE;
		if ( empty( $URBANISM_STORAGE[ $var_name ] ) ) {
			$URBANISM_STORAGE[ $var_name ] = 0;
		}
		$URBANISM_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'urbanism_storage_concat' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_concat( $var_name, $value ) {
		global $URBANISM_STORAGE;
		if ( empty( $URBANISM_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'urbanism_storage_get_array' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $URBANISM_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $URBANISM_STORAGE[ $var_name ][ $key ] ) ? $URBANISM_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $URBANISM_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $URBANISM_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'urbanism_storage_set_array' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_set_array( $var_name, $key, $value ) {
		global $URBANISM_STORAGE;
		if ( ! isset( $URBANISM_STORAGE[ $var_name ] ) ) {
			$URBANISM_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$URBANISM_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'urbanism_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function urbanism_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $URBANISM_STORAGE;
		if ( ! isset( $URBANISM_STORAGE[ $var_name ] ) ) {
			$URBANISM_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $URBANISM_STORAGE[ $var_name ][ $key ] ) ) {
			$URBANISM_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$URBANISM_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$URBANISM_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'urbanism_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_merge_array( $var_name, $key, $value ) {
		global $URBANISM_STORAGE;
		if ( ! isset( $URBANISM_STORAGE[ $var_name ] ) ) {
			$URBANISM_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$URBANISM_STORAGE[ $var_name ] = array_merge( $URBANISM_STORAGE[ $var_name ], $value );
		} else {
			$URBANISM_STORAGE[ $var_name ][ $key ] = array_merge( $URBANISM_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'urbanism_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $URBANISM_STORAGE;
		if ( ! isset( $URBANISM_STORAGE[ $var_name ] ) ) {
			$URBANISM_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			urbanism_array_insert_after( $URBANISM_STORAGE[ $var_name ], $after, $key );
		} else {
			urbanism_array_insert_after( $URBANISM_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'urbanism_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function urbanism_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $URBANISM_STORAGE;
		if ( ! isset( $URBANISM_STORAGE[ $var_name ] ) ) {
			$URBANISM_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			urbanism_array_insert_before( $URBANISM_STORAGE[ $var_name ], $before, $key );
		} else {
			urbanism_array_insert_before( $URBANISM_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'urbanism_storage_push_array' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_push_array( $var_name, $key, $value ) {
		global $URBANISM_STORAGE;
		if ( ! isset( $URBANISM_STORAGE[ $var_name ] ) ) {
			$URBANISM_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $URBANISM_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $URBANISM_STORAGE[ $var_name ][ $key ] ) ) {
				$URBANISM_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $URBANISM_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'urbanism_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $URBANISM_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $URBANISM_STORAGE[ $var_name ] ) && is_array( $URBANISM_STORAGE[ $var_name ] ) && count( $URBANISM_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $URBANISM_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $URBANISM_STORAGE[ $var_name ][ $key ] ) && is_array( $URBANISM_STORAGE[ $var_name ][ $key ] ) && count( $URBANISM_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $URBANISM_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'urbanism_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function urbanism_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $URBANISM_STORAGE;
		if ( ! isset( $URBANISM_STORAGE[ $var_name ] ) ) {
			$URBANISM_STORAGE[ $var_name ] = array();
		}
		if ( empty( $URBANISM_STORAGE[ $var_name ][ $key ] ) ) {
			$URBANISM_STORAGE[ $var_name ][ $key ] = 0;
		}
		$URBANISM_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'urbanism_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function urbanism_storage_concat_array( $var_name, $key, $value ) {
		global $URBANISM_STORAGE;
		if ( ! isset( $URBANISM_STORAGE[ $var_name ] ) ) {
			$URBANISM_STORAGE[ $var_name ] = array();
		}
		if ( empty( $URBANISM_STORAGE[ $var_name ][ $key ] ) ) {
			$URBANISM_STORAGE[ $var_name ][ $key ] = '';
		}
		$URBANISM_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'urbanism_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $URBANISM_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $URBANISM_STORAGE[ $var_name ] ) ? $URBANISM_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $URBANISM_STORAGE[ $var_name ] ) ? $URBANISM_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'urbanism_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function urbanism_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $URBANISM_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $URBANISM_STORAGE[ $var_name ]->$prop ) ? $URBANISM_STORAGE[ $var_name ]->$prop : $default;
	}
}
