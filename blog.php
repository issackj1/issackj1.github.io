...
if ( function_exists( 'urbanism_elementor_is_preview' ) && urbanism_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			urbanism_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	urbanism_new_main_query(
		array(
			'post_type'      => urbanism_get_theme_option( 'post_type' ),
			'category'       => urbanism_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'urbanism_filter_get_template_part', urbanism_blog_archive_get_template() ) );
}
